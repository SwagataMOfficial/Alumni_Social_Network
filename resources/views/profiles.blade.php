        ...
        <div class="px-8 mx-auto pt-3 flex justify-center gap-10">
            <div class="w-3/4">
                <div class="rounded-xl overflow-hidden relative mb-3">
                    ...
                    @if (str_contains($user['cover_picture'], 'default/'))
                        <img class="w-full h-40 object-cover" src="/storage/{{ $user['cover_picture'] }}"
                            alt="background image" id="profile-cover">
                    @else
                        <img class="w-full h-40 object-cover"
                            src="/storage/{{ $user['student_id'] }}/profile/{{ $user['cover_picture'] }}"
                            alt="background image" id="profile-cover">
                    @endif
                    ...
                    @if (str_contains($user['profile_picture'], 'default/'))
                        <img class="absolute top-16 left-8 z-10 w-32 aspect-square rounded-[50%] object-cover outline outline-slate-500"
                            src="/storage/{{ $user['profile_picture'] }}" alt="profile picture" id="profile-picture">
                    @else
                        <img class="absolute top-16 left-8 z-10 w-32 aspect-square rounded-[50%] object-cover outline outline-slate-500"
                            src="/storage/{{ $user['student_id'] }}/profile/{{ $user['profile_picture'] }}"
